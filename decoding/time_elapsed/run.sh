python predict_session_day.py --trial_type "animate" --task "memory"
python predict_session_day.py --trial_type "animate" --task "visual"
python predict_session_day.py --trial_type "animate" --task "visualsubset"
python predict_session_day.py --trial_type "animate" --task "visualsubset_easy"
python predict_session_day.py --trial_type "inanimate" --task "memory"
python predict_session_day.py --trial_type "inanimate" --task "visual"
python predict_session_day.py --trial_type "inanimate" --task "visualsubset"
python predict_session_day.py --trial_type "inanimate" --task "visualsubset_easy"

python predict_session_number.py --trial_type "animate" --task "memory"
python predict_session_number.py --trial_type "animate" --task "visual"
python predict_session_number.py --trial_type "animate" --task "visualsubset"
python predict_session_number.py --trial_type "animate" --task "visualsubset_easy"
python predict_session_number.py --trial_type "inanimate" --task "memory"
python predict_session_number.py --trial_type "inanimate" --task "visual"
python predict_session_number.py --trial_type "inanimate" --task "visualsubset"
python predict_session_number.py --trial_type "inanimate" --task "visualsubset_easy"
